...
This report presents a local variant of a library for *bytecode
construction and interpretation* written for the compiler construction
course IN5380 (formerly INF5110) at the University of Oslo.  The objective
of this exercise is to write a compiler for a simple example language. The
bytecode library and intepreter can be be used in the *code generation* and
...
translated into machine code before being run. A bytecode construction
