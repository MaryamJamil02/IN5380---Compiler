...
For a struct, there as analogous procedures ~public void ~addStruct(String
...
given the first indices from left to right, starting from ~0~. Then the
variables are given the subsequent indices in the order of their
declaration.
