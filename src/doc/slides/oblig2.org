...
2. *code generation* for ~compila22~ (ish) programs
...
*** Another glance at compila22                                     :B_frame:
    :PROPERTIES:
    :BEAMER_env: frame
    :END:
...
- interpreter and byte code library somewhat *limited*
   - cannot express full compila 22
   - no block structure
   - no reference types


- your delivery should support generating correct bytecode
  for the ~compila~ 22 source code file ~runme.cmp~
