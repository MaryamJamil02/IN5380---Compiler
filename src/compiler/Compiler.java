...
public class Compiler {
    ...
    public int compile() throws Exception {
        ...
        try {
            ...
            // If there is a SYNTAX ERROR (Should not get that for the tests):
            return 1;
        }
        ...
        try {
            ...
            // If there is a SEMANTIC ERROR (Should get that for the test with "_fail" in
            // the name):
    ...
    // Add standard library to symbol tabl
    public void addStandardLibrary(SymbolTable st) {
        ...
        st.addP("printint",
                new ProcDecl("printint", new ArrayList<>(List.of(intParam)), "int", null, new ArrayList<>()));
        st.addP("printfloat",
                new ProcDecl("printfloat", new ArrayList<>(List.of(floatParam)), "float", null, new ArrayList<>()));
        st.addP("printstr",
                new ProcDecl("printstr", new ArrayList<>(List.of(stringParam)), "string", null, new ArrayList<>()));
        st.addP("printline",
                new ProcDecl("printline", new ArrayList<>(List.of(stringParam)), "string", null, new ArrayList<>()));
    }

    private void generateCode(Program program) throws Exception {
        CodeFile codeFile = new CodeFile();
        program.generateCode(codeFile);
        System.err.println("done with generation in compiler");
        byte[] bytecode = codeFile.getBytecode();
        System.err.println("get bytecode from codefile");
