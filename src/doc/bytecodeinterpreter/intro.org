...
------------------------
its C\Gamma \Delta \Theta P\Sigma \Gamma \Upsilon \Theta \Delta \Phi \Sigma
\Theta objects, the actual bytecode can be extracted by using the procedure
\Psi \Theta \Omega Bff\Omega \Theta \Upsilon \Gamma \Delta \Theta () of a
C\Gamma \Delta \Theta F\Lambda \Xi \Theta object. The array of bytes that
is then created, is usually written to a file, which can then be read by
the virtual machine and run by its interpreter.
