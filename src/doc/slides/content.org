* Oblig 1                                           :B_ignoreheading:ARCHIVE:
  :PROPERTIES:
...
Determine if programs written in /Compila 21/ are syntactically correct: 
...
2. *code generation* for ~compila21~ (ish) programs
...
*** Another glance at compila21                                     :B_frame:
    :PROPERTIES:
    :BEAMER_env: frame
    :END:
...
- interpreter and byte code library somewhat *limited*
   - cannot express full compila 21
   - no block structure
   - no reference types


- your delivery should support generating correct bytecode
  for the ~compila~ 21 source code file ~runme.cmp~
