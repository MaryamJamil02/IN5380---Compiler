...
The constructore of the ~JMP~ class has an integer parameter ~jumpTo~. This
...
When library procedures are needed, they may be added to the ~CodeFile~
(and updated, the name alone is not enough), but no instruction should be
added, The interpreted recognizes the use of a library procedure by the
fact that it has no instruction in the binary file (~CodeFile~).
