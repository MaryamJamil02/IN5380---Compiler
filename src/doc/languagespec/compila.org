...
#+TEXINFO_DIR_TITLE:
#+TEXINFO_DIR_DESC:
#+TEXINFO_PRINTED_TITLE: Compila 18
#+DESCRIPTION: Compila 18 language specification
#+HTML_HEAD: <link rel="stylesheet" type="text/css" href="compilastyle.css" />
#+HTML_HEAD_EXTRA: <h1> Compila 18 language specification</h1>
...
of the language /Compila 18/.  The dynamic semantics, i.e., the description
...
Commas there are used as ``meta-meta symbols'' in the enumeration.



Here, ~{...}~ represents iteration of zero or more times, ~[...]~
...
``string-quotes'' (with a ~"~ at the beginning and the end) to distinguish
...
- /User-defined/ types: 
  ...
  2. Reference types, representing references to elements 
     of the specified types. The reference type cosntructor
     can be nested.
...
The language supports records. For people coming from Java/C++ etc, they
can be seen as (very) simple form of classes, containing only contain
instance variables as members, but neither supports /methods/ nor
...
dealing with records. Records do support /instantiation/ via the ~new~
keyword. Another aspect which resembles classes as in Java is that a
variables of record type contains either a pointer to an object of that
record type or the special pointer value ~null~.[fn:classesandrecords]
...
The language allows variables can be declared to be reference types, 
...
Correspondingly, you can follow a reference ~r~ by using ~deref(r)~, so
...
- The binary operations ~||~, ~&&~, ~+~, ~-~, ~*~, and ~.~ are
  /left-associative/, but exponetiation ~^~ is right-associative.

- Relation symbols are non-associative. That means that for example ~a < b + c < d~ is illegal.

- It's legal to write ~not not not b~, and it stands for ~(not (not (not
  b)))~.
...
#+include: "./code/example01.code" src compila
...
** Types and implicit type coversion

It is allowed to assign an expression of type ~int~ to a variable of type
~float~. The inverse situation is not allowed. There's no type cast
