...
\maketitle
\chapter{Oblig 1}
\label{sec:org2a8d871}



\section{Compila 20}
\label{sec:orge13c5e9}

\begin{frame}[label={sec:orgf295b0b}]{Oblig 1}
\begin{itemize}
...
\begin{frame}[label={sec:orgc6b2981}]{Goal (of oblig 1)}
\begin{block}{Parsing}
...
\begin{frame}[label={sec:orge190d40}]{Learning outcomes}
\begin{itemize}
...
\begin{frame}[label={sec:org847a120}]{Compila language at a glance}
...
\begin{frame}[label={sec:orgb46f708},plain]{Another glance}

\lstinputlisting[basicstyle=\scriptsize,emph={begin,end,in, proc,program,var,return,struct}]{code/swap.cmp}
\end{frame}


\begin{frame}[label={sec:org472392e},plain]{Grammar (1): declarations}
...
\begin{frame}[label={sec:org594cc89},plain]{Grammar (2): expressions, statements, etc.}
...
\begin{frame}[label={sec:orga61b95f},plain]{Grammar (3): statements and types}
...
\section{Tools}
\label{sec:orgdebb32a}

\begin{frame}[label={sec:org4ffacb9},fragile]{Tools: JFlex}
 \begin{itemize}
...
\begin{frame}[label={sec:orga1ac143}]{Sample lex code}
\includegraphics[width=\textwidth]{figures/snaps/lexcode}
\end{frame}



\begin{frame}[label={sec:org3a072c8},fragile]{CUP: Construction of useful parsers (for Java)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgaf6969c}]{Sample CUP code}
\includegraphics[width=\textwidth]{figures/snaps/cupcode}
\end{frame}



\begin{frame}[label={sec:orgfeafc68},fragile]{Build tool: ant}
...
\begin{frame}[label={sec:org436b98e}]{AST data structure}
...
\begin{frame}[label={sec:orge7f2cba},fragile]{Overview over the directory + first steps}
 \begin{itemize}
...
\begin{frame}[label={sec:org20d9ba7}]{Building: putting it together}
...
\section{Official}
\label{sec:org3d41d92}

\begin{frame}[label={sec:orgd8605ee},fragile]{Deadline}
...
\begin{frame}[label={sec:orge9f7453},fragile]{Deliverables}
 \begin{itemize}
...
\begin{frame}[label={sec:org53a93c4}]{Deliverables}
...
\begin{frame}[label={sec:org80db1c6},fragile]{Hand-in procedure}
 \begin{itemize}
...
\chapter{Oblig2}
\label{sec:orga210bca}


\begin{frame}[label={sec:org7542027},fragile]{Goal}
 \begin{enumerate}
...
\begin{frame}[label={sec:orgc3bc6de}]{Last time (O1)}
...
\begin{frame}[label={sec:org03cc40d}]{Learning outcome}
\begin{itemize}
...
\begin{frame}[label={sec:org696678f}]{Semantic analysis \& type checking}
\begin{itemize}
...
\begin{frame}[label={sec:orgf97eceb}]{Another glance at compila20}
...
\begin{frame}[label={sec:orgfd954c8},fragile,plain]{Type checking for conditionals}
 \begin{itemize}
...
\begin{frame}[label={sec:org003b0e5},fragile,plain]{Type checking: assignments}
 \lstset{language=java,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[label={sec:org4fb94fc},fragile]{Code generation}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf778bbf},fragile]{Code generation: limitations}
 \begin{itemize}
...
\begin{frame}[label={sec:org27feaf5},fragile,plain]{Code generation: creating a procedure}
 \lstset{language=java,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[label={sec:org26b9d61}]{Code generation: assignment}
\begin{center}
...
\begin{frame}[label={sec:orgf84df6b},fragile]{Testing}
 \begin{itemize}
...
\begin{frame}[label={sec:org332d8f2}]{Provided source code}
...
\begin{frame}[label={sec:orga74d730},fragile]{Provided source code}
...
\begin{frame}[label={sec:org5ffd983},fragile]{Deadline}
 \begin{alertblock}{Deadline}
12th May  20[19
\end{alertblock}
